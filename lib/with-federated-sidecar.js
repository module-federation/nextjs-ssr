  ...
  (nextConfig = {}) => {
    ...
    function startCompiler({
      ...
      const callback = (
        ...
        const { outputPath, assets, name } = info;
        if (name === "server") {
          log(
            federationPluginOptions.name,
            "starting to mutate asset emissions"
          );
          ...
          log(federationPluginOptions.name, "got stats");
          ...
          remoteContentPromise.then((remoteContent) => {
            log(federationPluginOptions.name, "got content");

            const newSource = remoteContent.replace(
              "init: () => (init)",
              `init: () => (init), chunkMap: () => (${statContent})`
            );
            log(federationPluginOptions.name, "created new source");

            log("########", path.join(outputPath, remoteEntry.name));
            fs.writeFile(
              path.join(outputPath, remoteEntry.name),
              newSource,
              (err) => {
            ...
            log(federationPluginOptions.name, "wrote new source");
          });
        } else {
    ...
    d.getElementsByTagName('head')[0].appendChild(script);
        ...
        if (!options.isServer) {
          ...
          config.plugins.push(
            new webpack.DefinePlugin({
              "process.env.MANIFEST": JSON.stringify(
                path.join(
                  options.dir,
