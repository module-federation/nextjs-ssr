...
const buildClientRemotes = (federationPluginOptions, webpack) => {
  const clientRemotes = Object.entries(
    ...
    // if using modern output, then there are no arguments on the parent function scope, thus we need to get it via a window global. 
      ...
      console.log(window[moduleName],'needs to be initialized')
      console.log(shareScope.default)
   ...
   return  ${remotePromiseLoad}(remoteConfig)
    })`;
    ...
    function startCompiler({
      ...
      const webpackOptions = {
        ...
        optimization: {
          ...
          // chunkIds:
          //   isProd || compilation.options.mode === "production"
          //     ? "deterministic"
          //     : compilerlation.options.optimization.chunkIds,
          runtimeChunk: false,
        },
      };
      ...
      console.log(webpackOptions.output.filename);
      ...
      const callback = (
        ...
        if (name === "federated-server" && experiments.flushChunks) {
          ...
          const statContent = JSON.stringify(
            require(path.join(outputPath, "../federated-stats.json"))
          );
          ...
          remoteContentPromise.then((remoteContent) => {
            const newSource = remoteContent.replace(
              "init: () => (init)",
              `init: () => (init), chunkMap: () => (${statContent}), hash: ()=>(${JSON.stringify(
                hash
              )})`
            );
            process.nextTick(() => {
